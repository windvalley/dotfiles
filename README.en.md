...
2. **Core dependencies**: Reads `Brewfile` and installs all CLI tools (stow, zellij, fish, helix, mise, aichat, go-musicfox, fzf, telnet, etc.) and GUI apps (Ghostty, OrbStack, Shottr, Thaw, JetBrains Mono font, etc.).
...
# macOS menu bar manager (Thaw, the official continuation fork of Ice after the original project went inactive; requires macOS 26+)
...
- `thaw`: The default macOS menu bar manager in this setup, provided by [Thaw](https://github.com/stonerl/Thaw) — the official continuation fork of Ice after the original project went inactive. The repository tracks the stable cask `cask "thaw"` and requires **macOS 26+**. For now, the repo manages installation only and does not directly version GUI runtime preferences such as `com.stonerl.Thaw.plist`. If you previously installed `jordanbaird-ice@beta`, migrate with `brew uninstall --cask jordanbaird-ice@beta` followed by `brew bundle --file="$HOME/dotfiles/Brewfile"`, so Ice and Thaw do not both fight for the menu bar at launch.
...
| `grs` | `git repo-stats` | Show a full repository overview including scale, activity, contributors, and object-store size |
...
- `git-repo-stats [summary|activity|contributors|refs|storage]`: Show a one-screen repository overview covering scale, activity, contributor rankings, hot paths, and `.git` object-store size; `contributors` can sort by commit count or total changed lines and shows commits, files, insertions, deletions, and net lines; supports `--scope head|default|all`, `--since 30d`, `--by commits|lines`, `--top 10`, and `--json`
