...
> - `colorscheme`: Zellij updates in real time; Ghostty needs `Cmd + Shift + ,` to reload config; Helix requires `:config-reload` for already opened buffers; Btop, Bat, and Delta take effect on the next run. The first switch to a repo-bundled custom syntect theme (currently `tokyonight` and `rose-pine`) triggers `bat cache --build` automatically; for Delta, `tokyonight` enables the official Tokyo Night feature, while `rose-pine` enables the repo-bundled Delta feature. Note: Bat and Delta still do not support `kanagawa`, `one-dark`, or `everforest`, so those tools are skipped automatically when switching to such themes.
