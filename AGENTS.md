...
- **`make restow`**：重新应用所有软链接。**当你在仓库中修改或新增了任何配置文件后，强烈推荐提示用户执行此命令**。
...
5. **任务完成后的用户引导**：当你更新了需要使用 stow 应用的配置后，由于你修改的是仓库内的源文件，**请在最终回复时提示用户手动运行 `make restow`** 或指导他们重载配置文件以使变更在实际系统中生效。