...
本文档为在当前代码库（`dotfiles`）中运行的各类 AI 编码助手、自动化代理与代码生成流程提供全局上下文、构建命令、代码规范和操作指南。请在执行任何读取或修改操作前仔细阅读并强制遵循本指南。

## 📦 项目背景与架构

本项目是一个高度定制化、基于 macOS 的现代终端开发环境配置文件集合。
- **核心工具栈**：Ghostty（终端容器）+ Zellij（终端复用与会话调度）+ Fish（友好的交互式 Shell）+ Helix（开箱即用的现代模态编辑器）+ Mise（全局与项目级环境/工具版本管理）+ AIChat（终端 AI 工作流入口）。
...
- **日志输出约定**：常规流程、成功、警告和错误信息应优先通过项目统一封装的彩色日志函数输出：
  - `info "..."` (蓝色，用于输出常规流程与提示)
  - `success "..."` (绿色，用于输出成功执行的结果)
  - `warn "..."` (黄色，用于输出警告与非致命异常)
  - `error "..."` (红色，用于输出致命错误，且通常伴随着 exit 1)
- **例外场景**：若输出的是 usage、机器可读结果、管道内容或 ANSI 控制序列，可按需使用 `printf` / `echo`。但不要绕开统一日志函数去打印普通流程提示。
...
项目根目录维护了一份严格的 `.editorconfig` 文件。你生成的任何新文件或代码段**必须**以该文件为准，不要用口头概括覆盖真实规则：
- **行尾序列 (End of Line)**：强制统一为 `LF`。
- **文件末尾换行**：所有文件末尾必须保留最终换行符（`insert_final_newline = true`），这表示“最后一行以换行结束”，**不是**额外插入一个空白行。
- **尾随空格**：每行末尾的多余空格必须移除（`trim_trailing_whitespace = true`）。
- **缩进规范**：
  - Markdown、Python 等文件维持 4 空格基线。
  - Shell、Fish、TOML、KDL、YAML、JSON、JS/TS 等文件按当前 `.editorconfig` 使用 2 空格。
...
- **私有隔离配置**：所有的隐私信息必须通过 `local/` 目录下的 `.example` 模板机制进行分离：
  ...
  - Ghostty 私有覆盖配置由 `local/ghostty.config.local.example` 生成并放置到用户目录对应位置。
...
1. **先搜索定位，再进行修改**：在你准备向任何配置追加 alias（别名）、设置环境变量、或添加新的快捷键之前，**你必须**先使用全文搜索工具（如 `rg`）或编辑器内搜索来检查代码库中是否已存在相似或冲突的定义。
   - *例如*：在增加新的 Fish alias 时，请先检索 `fish/dot-config/fish/` 目录；在给 Zellij 增加快捷键时，先检索 `zellij/dot-config/zellij/config.kdl`。
2. **遵守现有的生态习惯与极简主义设计**：
...
3. **Stow 软链映射的安全意识**：当需要修改配置时，务必注意文件所处的挂载状态。因为该环境可能已经执行过 `make stow`，很多 `~/.config/` 下的文件仅仅是软链接！
   - 你在修改配置时，**必须且只能**直接修改当前 dotfiles 仓库工作树中的源文件；不要直接修改 `~/.config/*`、`~/.local/*` 等 Stow 映射后的目标文件。
4. **提交变更前的闭环验证**：在你向用户宣称代码或配置修改完成前，确保自己已经执行过与改动类型匹配的验证。
   - 修改 Bash / Shell 脚本：运行 `make lint`。
   - 修改 Fish、Git、Zellij、Helix、Mise、Ghostty、Karabiner、AIChat 等受支持配置：优先运行 `./bin/validate-configs <tool>`，必要时运行 `make validate`。
   - 修改 `README.md` / `README.en.md` 且涉及标题层级：运行 `make docs`。
