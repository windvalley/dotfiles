...
2. **核心依赖**：读取 `Brewfile`，安装所有 CLI 工具（stow, zellij, fish, helix, mise, aichat, go-musicfox, fzf, telnet 等）与 GUI 应用（Ghostty, OrbStack, Shottr, Ice beta, JetBrains Mono 字体等）。
...
- `ice`: 默认集成的 macOS 菜单栏管理工具，仓库固定使用 `jordanbaird-ice@beta`，避免 `brew bundle` 回落到稳定轨道；当前仅托管安装入口，不直接托管 `com.jordanbaird.Ice.plist` 这类 GUI 运行期偏好。
