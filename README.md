...
8. **运行时安装**：通过 **Mise** 安装核心语言运行时（Go, Node, Bun, Python, Lua, Rust）、常用语言服务器与格式化工具（如 gopls、pyright、lua-language-server、stylua、vtsls 等），以及开箱即用的基础 CLI 工具（gh, bat, eza, fd, ripgrep, glow, shellcheck、codex、claude-code 等）。
...
依赖链接完成后，通过 `mise` 统一拉取所有配置好的工具链（包括运行时、LSP 以及 `codex` / `claude-code` / `opencode` 这类 AI CLI）：
...
本配置将常用运行时（Go/Node/Bun/Python/Lua/Rust）、语言服务器、Formatter，以及 `codex` / `claude-code` / `opencode` 这类 AI CLI **全部统一收敛交由 mise 管理**，实现两层优雅隔离：
