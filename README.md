...
- `local/`: 本地环境私有配置模板（用于 Fish 环境变量脱敏、Git 多账号隔离、Ghostty 私有覆盖以及 go-musicfox 本地配置初始化）
- `Makefile`: 自动化构建与维护脚本
...
2. **核心依赖**：读取 `Brewfile`，安装所有 CLI 工具（stow, zellij, fish, helix, mise, aichat, go-musicfox, fzf, telnet 等）与 GUI 应用（Ghostty, OrbStack, Shottr, Thaw, JetBrains Mono 字体等）。
...
8. **运行时安装**：通过 **Mise** 安装核心语言运行时（Go, Node, Bun, Python, Lua, Rust）、常用语言服务器与格式化工具（如 gopls、pyright、lua-language-server、stylua、vtsls 等），以及开箱即用的基础 CLI 工具（gh, bat, eza, fd, ripgrep, glow, shellcheck、codex、kimi、claude-code 等）。
9. **隐私配置模板**：自动在用户目录创建 Git 信息模板（`.gitconfig.local`/`.work`）、Fish 私密环境变量模板（`.fish.local.fish`）、Ghostty 私有配置模板（`.ghostty.local`）以及 go-musicfox 的本地配置模板（`~/.local/share/go-musicfox/config.toml`）。
...
# macOS 菜单栏管理工具（Thaw，Ice 在原作者停更后的官方接续 fork，要求 macOS 26+）
...
- `telnet`: 提供经典的 TCP 连通性测试命令，适合快速验证目标主机与端口是否可达。
...
- `thaw`: 默认集成的 macOS 菜单栏管理工具，是 [Thaw](https://github.com/stonerl/Thaw)（Ice 在原作者停更后的官方接续 fork）。仓库使用稳定轨道 `cask "thaw"`，要求 **macOS 26+**；当前仅托管安装入口，不直接托管 `com.stonerl.Thaw.plist` 这类 GUI 运行期偏好。若你之前装的是旧的 `jordanbaird-ice@beta`，迁移时请先 `brew uninstall --cask jordanbaird-ice@beta` 再 `brew bundle --file="$HOME/dotfiles/Brewfile"`，避免两者同时启动争抢菜单栏。
...
依赖链接完成后，通过 `mise` 统一拉取所有配置好的工具链（包括运行时、LSP 以及 `codex` / `kimi` / `claude-code` / `opencode` 这类 AI CLI）：
...
`go-musicfox` 的活跃配置不适合像普通 dotfiles 一样直接 Stow 到仓库里，因为它会在运行过程中写入登录态、Cookie、缓存路径等私有状态。为避免这些内容回流到 Git 工作树，本项目采用“仓库内模板 + 仓库外实际配置”的方式集成：

1. Fish 会统一导出 `MUSICFOX_ROOT="$HOME/.local/share/go-musicfox"`，把 go-musicfox 的配置、缓存和运行状态固定到仓库外目录。
2. `install.sh` 会在首次安装时自动复制模板 `~/dotfiles/local/go-musicfox.config.toml.example` 到 `~/.local/share/go-musicfox/config.toml`。
3. 后续请直接编辑 `~/.local/share/go-musicfox/config.toml`，不要把登录后的真实 Cookie 或账号状态提交回仓库。

模板默认做了两项保守调整：

- 显式使用 `player.engine = "osx"`，更贴合 macOS 媒体控制链路。
- 默认关闭 `enableMouseEvent`，减少 Ghostty/Zellij 组合下的鼠标事件抢占。

> [!NOTE]
> Fish 与 Ghostty 的本地私有文件现在直接放在 `$HOME` 根目录（`~/.fish.local.fish`、`~/.ghostty.local`）；go-musicfox 的私有配置与运行状态位于 `~/.local/share/go-musicfox/`。它们都物理上独立于 dotfiles 仓库，不会随着 Stow 软链写回仓库工作树。
...
| `mfx` | `musicfox` | 启动 go-musicfox 网易云音乐终端客户端 |
...
| `grs` | `git repo-stats` | 全面查看当前仓库的规模、活跃度、贡献者和对象库存储体量 |
...
本配置将常用运行时（Go/Node/Bun/Python/Lua/Rust）、语言服务器、Formatter，以及 `codex` / `kimi` / `claude-code` / `opencode` 这类 AI CLI **全部统一收敛交由 mise 管理**，实现两层优雅隔离：
...
- `git-repo-stats [summary|activity|contributors|refs|storage]`: 一屏总览当前仓库的规模、活跃度、贡献者排行、热点目录和 `.git` 对象库存储体量；其中 `contributors` 支持按提交数或总代码变更量排序，并展示提交数、文件数、新增/删除/净增行数；支持 `--scope head|default|all`、`--since 30d`、`--by commits|lines`、`--top 10` 与 `--json`
...
- `validate-configs [tool|all]`: 验证配置文件语法和完整性（支持 fish/git/zellij/helix/mise/ghostty/karabiner/aichat/go-musicfox）
