# Helix 快速上手指南 (Neovim 用户版)

如果你是一名 Neovim 重度用户，刚切换到 Helix，这份文档将帮助你快速度过"肌肉记忆"的磨合期。

## 1. 核心思维转变：Selection-First

| | Neovim (Verb → Object) | Helix (Object → Verb) |
| :--- | :--- | :--- |
| 删除单词 | `dw` (先说"删除"，再说"单词") | `wd` (先选中单词，再删除) |
| 删除行 | `dd` | `xd` (`x` 选中行，`d` 删除) |
| 复制行 | `yy` | `xy` |

**为什么？** 在 Helix 中，你先选择文本（高亮可见），再执行动作。按下删除键之前就能**看到**受影响的范围，无需猜测。

## 2. 三种核心模式

- **Normal** — 默认模式。移动光标即是在"选择"（即使只选中一个字符）。
- **Insert** — `i` / `a` / `I` / `A` 进入，与 Vim 一致。
- **Select** — `v` 进入。移动操作会**扩展**选区而非移动光标（等同 Vim Visual Mode）。

## 3. 按键映射对比

### 基础编辑

| 操作 | Neovim | Helix | 备注 |
| :--- | :--- | :--- | :--- |
| 行首 / 行尾 | `0` / `$` | `gh` / `gl` | `g` 菜单 (Go) |
| 文件末尾 | `G` | `ge` | |
| 查找字符 | `f` + 字符 | `f` + 字符 | 一致 |
| 搜索 | `/text` | `/text` Enter | `n` / `N` 跳转依然有效 |
| 替换 | `:%s/old/new/g` | `%` → `s` → 输入正则 → `c` → 输入新内容 | 全文多光标替换 |
| 注释 | `gcc` / `gc` | `Ctrl + c` | 选中多行后批量注释 |
| 悬浮文档 | `K` | `Space + k` | `K` 在 Helix 中默认为 `keep_selections`（筛选多光标） |
| 代码动作 | `leader + ca` | `Space + a` | `Space` 是 Helix 的 leader 菜单 |
| 撤销 / 重做 | `u` / `Ctrl + r` | `u` / `U` | |
| 大小写转换 | `~` (单字符) | `~` | 选中文本后按 `~` 批量转换 |

### 插入模式

虽然 Helix 提倡在 Normal 模式移动，但为了方便 Emacs/macOS 用户，已配置以下快捷键：

| 按键 | 功能 | 类型 |
| :--- | :--- | :--- |
| `Ctrl + f` / `b` | 光标向右 / 向左移动字符 | 自定义 |
| `Ctrl + n` / `p` | 光标向下 / 向上移动行 | 自定义 |
| `Ctrl + a` / `e` | 跳转到行首 / 行尾 | 自定义 |
| `Ctrl + d` | 向后删除字符 | 原生 |
| `Ctrl + k` | 删除至行尾 | 原生 |
| `Ctrl + u` | 删除至行首 | 原生 |
| `Ctrl + w` | 向前删除单词 | 原生 |
| `Ctrl + j` | 插入新行 | 原生 |

### 选择操作

Helix 的光标本身就是一个选区（至少 1 字符宽）。部分移动（如 `w`/`f`/`t`）会自动选中经过的文本，而 `g` 系列（`gh`/`gl`/`ge`）只是移动光标、不产生选区。

**`v` 键是关键粘合剂**：按 `v` 进入选择模式后，**所有**移动都会扩展选区——包括原本不选中文本的 `g` 系列。例如 `vgl` 选中到行尾、`vge` 选中到文件末尾。

| 按键 | 功能 | Vim 类比 |
| :--- | :--- | :--- |
| `v` | 进入选择模式（移动时扩展选区） | `v` |
| `w` / `b` / `e` | 选中到下一个词首 / 上一个词首 / 词尾 | 移动 + 自动选中 |
| `x` | 选中当前行（连按扩展多行） | `V` |
| `%` | 选中整个文件 | `ggVG` |
| `mi` / `ma` + 对象 | 选中内部 / 含分隔符的文本对象（`miw`、`ma(`） | `vi` / `va` |
| `mm` | 跳转到配对括号（再按一次跳回） | `%` |
| `maf` / `mif` | 选中整个函数 / 函数内部 | — |
| `maa` / `mia` | 选中整个参数 / 参数内部 | — |
| `Space + o` / `i` | 语法树向外扩展 / 向内收缩（自定义，替代 `A-o`/`A-i`） | — |
| `;` | **折叠选区**为单个光标点 | — |
| `,` | **取消多光标**，只保留主选区 | — |

> [!IMPORTANT]
> **`;`（折叠选区）是 Helix 中使用频率最高的按键之一。** 因为几乎所有操作都会产生选区，你需要频繁按 `;` 把选区折叠回单个光标。

### 多光标操作

不需要插件，Helix 原生支持多光标：

1. `x` 选中多行
2. `s` 输入正则（如 `foo`），在选区内匹配子选区
3. `K` 输入正则，**只保留匹配的选区**（过滤不需要的）
4. `c` 统一修改 — 这就是 Helix 的"查找替换"

| 按键 | 功能 |
| :--- | :--- |
| `s` | 在选区内按正则匹配子选区 |
| `S` | 按正则拆分选区 |
| `C` | 复制选区到下一行（创建多光标） |
| `K` | 按正则**保留**匹配的选区（过滤掉不需要的） |

### 寄存器（命名剪贴板）

`y` / `p` 默认使用内部寄存器。用 `"` + 名称可指定寄存器：

| 按键 | 功能 |
| :--- | :--- |
| `"ay` | 复制到寄存器 `a` |
| `"ap` | 从寄存器 `a` 粘贴 |
| `"bd` | 剪切到寄存器 `b` |
| `Space + y` / `Space + p` | 复制到 / 粘贴自**系统剪贴板** |

> [!TIP]
> 默认寄存器与系统剪贴板隔离（已配置），编辑过程中的临时复制不会冲掉剪贴板里的重要内容。需要与浏览器等外部程序交互时，用 `Space + y/p`。

### 宏录制

当多光标无法覆盖的重复操作（如 50 行格式各异的修改），用宏录制一次、回放 N 次：

| 按键 | 功能 |
| :--- | :--- |
| `Q` | 开始 / 停止录制（状态栏显示 `recording`） |
| `q` | 回放宏（`10q` = 回放 10 次） |

> [!NOTE]
> 宏默认录制到 `@` 寄存器，不会与剪贴板冲突。

## 4. LSP 与代码导航

已在 `languages.toml` 中配置多种语言，以下快捷键开箱即用：

| 按键 | 功能 |
| :--- | :--- |
| `gd` | 跳转到定义 (Go to Definition) |
| `gr` | 查看引用 (Go to References) |
| `gy` | 跳转到类型定义 |
| `Space + r` | 重命名符号 |
| `Space + a` | 代码动作 |
| `Space + k` | 悬浮文档 |
| `Space + p` | 从系统剪贴板粘贴 (Paste from system) |
| `Space + m` | Markdown 预览 (Glow) |

## 5. 窗口与缓冲区

| 按键 | 功能 | Vim 类比 |
| :--- | :--- | :--- |
| `Ctrl + w, v` | 纵向分屏 | `:vsplit` |
| `Ctrl + w, s` | 横向分屏 | `:split` |
| `Ctrl + w, w` | 切换窗口 | `Ctrl + w, w` |
| `Ctrl + w, q` | 关闭窗口 | `:q` |
| `Space + b` | 缓冲区选择器（推荐，带模糊搜索） | `:ls` + `:b` |
| `Space + f` | 文件选择器 | Telescope |
| `gn` / `gp` | 下一个 / 上一个缓冲区 | `:bn` / `:bp` |
| `:bc` | 关闭当前缓冲区 | `:bd` |

> [!TIP]
> 开启 `bufferline = "multiple"`（已配置）后，打开多个文件时顶部会显示缓冲区标签栏，用 `gn`/`gp` 快速切换。

## 6. 渐进式发现

忘记快捷键？按下以下任意键后**稍等片刻**，Helix 会弹出所有可用选项：

`g`（跳转）· `m`（匹配）· `z`（视图）· `[` / `]`（导航）· `Space`（leader 菜单）

## 7. 实用命令

| 命令 | 功能 |
| :--- | :--- |
| `:config-open` | 打开配置文件 |
| `:config-reload` | 重载配置 |
| `Ctrl + r` | 重载当前文件（自定义映射，因为 Helix 默认不会自动重载被外部修改的文件） |
| `:tutor` | **必读！** 交互式教程（约 15 分钟） |
