...
### Added
- 新增统一的 AI 路由机制及专用本地指令 `q`，并显著优化了 Aichat/Ollama 后端服务的错误诊断与格式化输出。

### Changed
- 优化 Shell 环境迁移逻辑，重点改进了从 Zsh 切换至 Fish 时的 PATH 路径同步与初始化稳定性。
- 增强安装脚本的系统兼容性，强制规范工具链依赖（如 gsed），并优化了编辑器的调用逻辑。
